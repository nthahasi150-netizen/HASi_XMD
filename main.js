...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [HASi_XMD] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'HASi_XMD' };
            }
        });
            ...
            try {
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: '© HASi_XMD',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:HASi_XMD BOY\nORG:HASi_XMD BOY;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© HASi_XMD' } }
                    }},
        ...
        arslanLog(`HASi_XMD Pair error: ${err.message}`, 'error');
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 HASi_XMD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
